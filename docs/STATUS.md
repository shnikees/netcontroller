...
| Batch replay | A folder of recordings processed in one pass, four at a time, skipping what is already done and what is still being written. 32 nets, 4582 transmissions |
| Roster biasing | The roster reaches Whisper as hotwords rather than a prompt -- measured at more than twice the trusted recoveries on real net audio, switchable back with `whisper.bias_mode` |
